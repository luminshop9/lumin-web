...
try:
    if GOOGLE_CREDS:
        ...
        creds_str = GOOGLE_CREDS.replace('\n', '').replace('\r', '').strip()
...
def parse_decimal(val):
    ...
    s = re.sub(r'[^\d,.]', '', s)
    # Si hay coma y punto, el último separador gana
    if ',' in s and '.' in s:
        if s.rfind(',') > s.rfind('.'):
            s = s.replace('.', '').replace(',', '.')
        else:
...
def api_ventas_hoy():
    ...
    try:
        ...
        print("Error en /api/ventas/hoy:", e)
        return jsonify({"total": 0, "cantidad_items": 0, "transacciones": 0})
...
def api_stock_bajo():
    ...
    try:
        ...
        print("Error en /api/stock/bajo:", e)
        return jsonify([])
...
def api_ganancias_resumen():
    ...
    try:
        ...
        print("Error en /api/ganancias/resumen:", e)
        return jsonify({})
...
def api_actualizar_producto():
    ...
    try:
        ...
        print("Error en /api/actualizar-producto:", e)
        return jsonify({"error": str(e)}), 500
...
def api_guardar_cliente():
    ...
    try:
        ...
        if hoja is None:
            ...
            return jsonify({"error": "Hoja clientes no encontrada. Créala con columnas: id, nombre, telefono, email, fecha_registro"}), 404
...
def api_anular_venta():
    ...
    try:
        ...
        print("Error en /api/anular-venta:", e)
        return jsonify({"error": str(e)}), 500
